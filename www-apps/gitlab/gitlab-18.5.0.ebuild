...
DEPEND="
	...
	>=dev-vcs/gitlab-shell-14.45.3[relative_url=]
...
src_install() {
	...
	# Note: Relative paths are relative to GitLab Rails.root i.e. ${GITLAB}
	#       and dodir prepends ${ED} automatically to the install location.
	[[ $backup_path == /* ]] || backup_path="${GITLAB}/${backup_path}"
	dodir ${backup_path}
...
pkg_postinst() {
	...
	if [ "$MODUS" = "new" ]; then
		...
		elog "       su -l postgres"
		elog "     GitLab needs three PostgreSQL extensions: amcheck, pg_trgm, btree_gist, plpgsql."
		elog "     To create the extensions if they are missing do:"
		elog "       psql -d template1 -c \"CREATE EXTENSION IF NOT EXISTS amcheck;\""
...
pkg_config_do_upgrade_migrate_database() {
	elog "Migrating database: Run the post-deployment migrations ..."
	su -l ${GIT_USER} -s /bin/sh -c "
		cd ${GITLAB}
		${BUNDLE} exec rake gitlab:db:configure RAILS_ENV=${RAILS_ENV}" \
			|| die "failed to migrate database."
}
