...
It may be useful to create a \branch{project} branch to develop multiple \eBuild{s} concurrently for a large project.
Once the project is working one would split it up into the various \branch{\package[category]{package}} branches.
Similarly one may leverage \branch{project} branches to combine multiple \eBuild{s} together before pushing them into \branch{master}.
